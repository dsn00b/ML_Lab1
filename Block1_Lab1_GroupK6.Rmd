...
plot(lasso3_3_for_plot, xvar = "lambda")
title("LASSO model coefficent convergence", line = 3)
...
plot(lasso3_3_for_plot, "lambda")
title(main = "Lasso regression: Coef vs log lambda", line = 3)

plot(lasso3_5_for_plot, "lambda")
title(main = "Ridge regression: Coef vs log Lambda", line = 3)
```
...
title(main = "Cross-validation 3 fold CI:s for MSE vs values of lambda", line = 3)
...
#calculating new sigma used for generation
...
easy_ids <- tail(sortedResult$ix, n=c(2)) #179 183
plotHeatmap <- function(index){
  heatmap(matrix(as.numeric(train_eight[index, 1:64]), 
                 8, 8, byrow = T), Colv = NA, Rowv = NA)
}
...
plot(1:30, e_v, ylim = c(0,0.06), type = "l", col = "red", xlab = "K", 
     ylab = "miss-classification error rate", axes = FALSE)
axis(1, at = seq(1, 30, by = 1))
axis(2)
lines(1:30, e_t, ylim = c(0,0.06), col = "blue")
abline(v = 4, lty = 3)
...
for (k in 1:30) {
  s <- s2 <- 0
  for (i in 0:9) {
    s <- s + sum(log(kv[[k]]$prob[which(validation$number==i), 
                                  toString(i)] + 1e-15)/nrow(kv[[k]]$prob))
    s2 <-s2 + sum(log(kt[[k]]$prob[which(train$number==i), 
                                   toString(i)] + 1e-15)/nrow(kt[[k]]$prob))
  }
  r_v[k] <- -s
  r_t[k] <- -s2
}
plot(1:30, r_v, type = "l", col = "red", xlab = "K", 
     ylab = "mean cross entropy", axes = FALSE, ylim = c(0,1))
axis(1, at = seq(1, 30, by = 1))
axis(2)
lines(1:30, r_t, col = "blue")
abline(v = 6, lty = 3)
legend(22, 0.9, legend=c("validation", "train"), 
...
plot_data <- data.frame("log_lambda" = l
                        og(lasso3_3_for_plot$lambda), "df" = lasso3_3_for_plot$df )
ggplot(data = plot_data ,aes( x= log_lambda , y = df)) + geom_point() + 
  labs(title = "Degrees of freedom vs Log lambda", 
       x = "Log lambda", y = "Degrees of freedom")
...
ggplot(plot_data_36, aes(x = y_test, y = s0)) + geom_point() + 
  labs(title = paste("Comparison of Predicted values with LASSO optimal vs",
                     "\n observed values from the test data"), 
       x = "y from test data", y = "Predicted values from LASSO")
...
generated_data <- data.frame("ytest" = test_fat[,101], "generated" = new_targets )
ggplot(generated_data, aes(x = ytest, y = generated)) + geom_point()+
  labs(title = "Generated data vs observed test data", 
       x = "y from test data", y = "Generated data")
